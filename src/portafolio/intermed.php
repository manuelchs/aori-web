<!DOCTYPE html>
<html lang="es">
<head>
    <?php include("../inc/head.html") ?>
    <title>AORI Portafolio - Intermed</title>
</head>
<body>
    <?php include("../inc/lightbox.html"); ?>
    <?php include("../inc/navbar.html") ?>
    <section class="desfase-navbar hero-portafolio intermed">
        <div class="info-project">
            <div class="logo-icons">
                <img class="project-logo" src="#/img/portafolio/intermed-logo.png" alt="">
                <div class="project-icons">
                    <div class="icon-wrapper">
                        <img src="#/img/branding.png" alt="">
                        <span>Branding</span>
                    </div>
                    <div class="icon-wrapper">
                        <img src="#/img/web.png" alt="">
                        <span>Desarrollo Web & Apps</span>
                    </div>
                </div>
            </div>
            <div class="project-desc">
                <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat. Ut wisi enim a</p>
            </div>
        </div>
    </section>
    <section class="project">
        <div class="main-wrapper">
            <div class="project-stage">
                <h2 class="title">desarrollo de marca</h2>
                <div class="stage-img">
                    <img class="lightbox-item bg" src="#/img/portafolio/intermed/intermed-1.png" alt="">
                </div>
                <div class="stage-desc">
                    <p>Conceptualización y desarrollo de logo.</p>
                    <div class="marco"></div>
                </div>
            </div>
            <div class="project-stage">
                <h2 class="title">app</h2>
                <div class="stage-img">
                    <img class="lightbox-item" src="#/img/portafolio/intermed/intermed-2.png" alt="">
                </div>
                <div class="stage-desc">
                    <p>Desarrollo de diseño responsivo y adaptable</p>
                    <div class="marco"></div>
                </div>
            </div>
        </div>
    </section>
    <?php include("../inc/portafolio-slider.php") ?>
    <?php include("../inc/footer.html") ?>
</body>
</html>